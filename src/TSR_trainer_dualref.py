...
class TrainerForContinuousEdgeLine:
    ...
    def train(self, loaded_ckpt):
        ...
        use_apex_amp = self.config.AMP and apex_amp is not None
        use_torch_amp = self.config.AMP and not use_apex_amp
        scaler = GradScaler(enabled=use_torch_amp)

        if use_apex_amp:  # Apex AMP
            model, optimizer = apex_amp.initialize(model, optimizer, opt_level=config.amp_opt_level)
        elif self.config.AMP and self.global_rank == 0:
        ...
        for epoch in range(config.max_epochs):
            ...
            for it, items in enumerate(tqdm(loader, disable=not_show_tqdm)):
                ...
                with autocast(enabled=use_torch_amp):
                    edge, line, loss = model(
                        items['img'], items['edge'], items['line'], items['edge'], items['line'], items['mask'],
                ...
                model.zero_grad()
                if use_apex_amp:
                    with apex_amp.scale_loss(loss, optimizer) as loss_scaled:
                        loss_scaled.backward()
                    torch.nn.utils.clip_grad_norm_(apex_amp.master_params(optimizer), config.grad_norm_clip)
                    optimizer.step()
                elif use_torch_amp:
    ...
    def train(self, loaded_ckpt):
        ...
        for epoch in range(config.max_epochs):
            ...
            for it, items in enumerate(tqdm(loader, disable=not_show_tqdm)):
                ...
                with autocast(enabled=use_torch_amp):
                    edge, line, loss = model(
                        items['mask_img'], items['edge'], items['line'], items['edge'], items['line'],
                        items['erode_mask'],
                        global_img=items['g_img'], global_edge=items['g_edge'], global_line=items['g_line'],
                        local_img=items['l_img'], local_edge=items['l_edge'], local_line=items['l_line'],
                        local_mask=items['l_mask'])
